...
# Difference 2: Without @tool, we must MANUALLY define thw JSON schema for each function.
# This is exactly what Langchain's @tool decorator generates automatically
# from function's type hints and docstring.
tools_for_llm = [
    {
        "type": "function",
        "function": {
...
# However, this requires your docstrings to follow the Google docstring commands
# so Ollama can parse parameters description from thw Args section. For example:
#   def get_product_price(product: str)-> float:
...
def run_agent(question:str):
    ...
    for iteration in range(1, MAX_ITERATIONS+1):
        ...
        # Difference 5. ollama_chat_traced(messages=messages)
        ...
        # Process only the FIRST tool call - force one tool iteration
