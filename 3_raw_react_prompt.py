...
# --- Tools ollama @traceable decorator ---
...
def apply_discount(price: float, discount_tier: str)->float:
    """Apply the discount_tier to a price and return the final price.
...
tools = {                                       # we are not using JSON format for function calling but Raw text using import re, inspect
    "get_product_price": get_product_price,
    "apply_discount": apply_discount,
}

# CHANGE 3: Delete the JSON schemas. Tools now live inside the prompt as plain text.
# We derive descriptions from functions themselves using inspect.
...
# CHANGE 4: Drop tools =  form ollama.chat(). The LLM has no idea it's an agent -
# all agency from the prompt above and our regex parsing below
